...
final class Reflection
{
    ...
    public static function metaProperties(UnitEnum $enum): array
    {
        ...
        $traits = array_values(array_filter($reflection->getTraits(), fn (ReflectionClass $class) => $class->getName() !== 'Cline\Enums\Metadata'));

        $traitsMeta = array_map(
            fn (ReflectionClass $trait) => self::parseMetaProperties($trait),
            $traits,
        );
